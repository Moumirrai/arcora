...
import type { Displacement } from "../../../src/core/types";
...
describe("Node class", () => {
  it("generates a uuid v4 id when none provided", () => {
    ...
    const node2 = new Node(modelMock, { coords: { x: 3, z: 4 } });
  ...
  it("pos getter returns coordinates and returned object is independent", () => {
    const node = new Node(modelMock, { coords: { x: 5, z: 6 } });
    const pos = node.pos;
  ...
  it("toData includes optional fields when present and omits when absent", () => {
    const initialDisplacement: Displacement = { u: 0.1, v: 0.2 };
    const node = new Node(modelMock, {
      coords: { x: 3, z: 4 },
      id: "id-1",
      prescribedDisplacement: initialDisplacement,
      name: "Node A",
    });
    ...
    expect(data.prescribedDisplacement).toBe(initialDisplacement); // reference preserved
    ...
    const node2 = new Node(modelMock, { coords: { x: 0, z: 0 }, id: "id-2" });
  ...
  it("toData serializes and deserializes correctly", () => {
    const initDisp: Displacement = { u: 1, v: 2 };
    const node = new Node(modelMock, {
      coords: { x: 3, z: 4 },
      prescribedDisplacement: initDisp,
      name: "Node A",
    });
